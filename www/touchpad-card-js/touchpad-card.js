var vibrate = function (detail) {
  var event = new Event('haptic', {
      bubbles: false
  });
  event.detail = detail;
...
class touch extends HTMLElement {
  ...
  set hass(hass) {
    ...
    if(typeof(document.querySelector('ha-card')) === 'undefined' || document.querySelector('ha-card') === null){
      //load css
    this.innerHTML = `<head><link rel="stylesheet" href="/local/touchpad-card-js/style.css"></head>`;
      //card creation
...
function touchMove(event,ha,ent) {
  ...
  if (Math.abs(diffX) > Math.abs(diffY)) {
    ...
    if (diffX > 0) {
      ...
      vibrate(window,'haptic','selection');
    } else {
      // swiped right
